...
def tomar_captura_kibana(page, url, nombre_archivo, output_dir="output"):
    ...
    # FORZAR RELOAD para que Kibana actualice el contenido
    print("Refrescando página para actualizar vista...")
    page.reload(wait_until='networkidle', timeout=60000)

    # ESPERA INTELIGENTE
    print("Esperando que cargue el dashboard...")

    # Esperar un selector más genérico de Kibana
    try:
...
def ejecutar_capturas_batch(urls, output_dir="output"):
    ...
    with sync_playwright() as p:
        ...
        if "login" in page.url:
            ...
            page.wait_for_url(primera_url, timeout=30000)
            print("Login exitoso!")
        
        # PROCESAR TODAS LAS URLs CON LA SESIÓN AUTENTICADA
        for i, (nombre, url) in enumerate(urls.items(), 1):
            print(f"\n[{i}/{len(urls)}] Procesando: {nombre}")
            try:
                tomar_captura_kibana(page, url, nombre, output_dir)
